...
export class WebGPURenderer {
    ...
    try {
      ...
      if (typeof mql.addEventListener === 'function') {
        mql.addEventListener('change', () => {
          try {
            this.clearColor = getClearColorFromCSS();
          } catch (e) {}
        });
      } else if (typeof (mql as any).addListener === 'function') {
        // fallback for older browsers
        (mql as any).addListener(() => {
          try {
            this.clearColor = getClearColorFromCSS();
          } catch (e) {}
        });
  ...
  private createAssets() {
    ...
    // 初始化纹理加载器
    this.textureLoader = new TextureLoader(this.device, this.textureArray, IMAGE_URLS.length);
  }
  ...
  public async addParticle() {
    ...
    while (cnt-- > 0) {
      ...
      try {
        ...
        // 写入动态数据到两个缓冲区（初始状态相同）
        this.device.queue.writeBuffer(this.dynamicBuffers[0], this.particleCount * DYNAMIC_PARTICLE_STRIDE, dynamicData.buffer);
        this.device.queue.writeBuffer(this.dynamicBuffers[1], this.particleCount * DYNAMIC_PARTICLE_STRIDE, dynamicData.buffer);
